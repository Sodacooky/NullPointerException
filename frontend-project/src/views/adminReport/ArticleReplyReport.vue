<template>
  ...
  <el-table :data="tableData">
    ...
    <el-table-column label="操作" min-width="96px">
      <template #default="scope">
        <el-button
          @click="openDetailDialog(scope.row.id, scope.row.goalArticleReplyId)"
        >
          详情与处理
        </el-button>
      </template>
  ...
  <el-dialog
    ...
    width="800px"
  >
    <template #header><span style="font-weight: bold">回复详情</span></template>
...
export default {
  ...
  methods: {
    ...
    openDetailDialog(reportId, replyId) {
    ...
    doRemove() {
      this.$confirm("删除这条回复？", "确定删除", {
        ...
        .then(() => {
          //确认删除
          AdminContentManageApi.removeArticleReply(
            this.detailDialogData.id
          ).then((resp) => {
            if (resp.data.code === "0") {
              this.$notify.success("操作成功");
