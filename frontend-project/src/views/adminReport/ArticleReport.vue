<template>
  ...
  <el-table :data="tableData">
    <el-table-column label="标题" min-width="128px" prop="goalArticleTitle" />
    <el-table-column
      label="正文摘要"
      min-width="256px"
      prop="goalArticleShortText"
    />
    ...
    <el-table-column label="操作" min-width="96px">
      <template #default="scope">
        <el-button
          @click="openDetailDialog(scope.row.id, scope.row.goalArticleId)"
        >
          详情与处理
        </el-button>
      </template>
  ...
  >
    ...
    <template #footer>
      <el-button type="primary" @click="doArticleUpdate()">更改</el-button>
...
export default {
  ...
  methods: {
    fetch() {
      AdminReportApi.getArticleReport(
        this.page,
    ...
    openDetailDialog(reportId, articleId) {
      this.isShowDetailDialog = true;
      ReadingApi.getArticle(articleId).then((resp) => {
    ...
    doArticleUpdate() {
      AdminContentManageApi.modifyArticle(
    ...
    doRemove() {
      this.$confirm(
        "删除文章将会把其下的回复也一并删除，确定吗？",
        "确定删除",
        {
          confirmButtonText: "是的，连同回复一起删除",
          cancelButtonText: "取消",
          type: "warning",
        }
      )
        .then(() => {
          //确认删除
          AdminContentManageApi.removeArticle(this.detailDialogData.id).then(
            (resp) => {
              if (resp.data.code === "0") {
                this.$notify.success("操作成功");
              } else {
