<template>
  ...
  <el-container>
    <el-main>
      ...
      <!--  结果列表-->
      <div class="search-result-area" v-if="searchText.length>0">
        <!--      切换结果类型按钮-->
        <div>
          <div style="margin-top: 8px">
            <el-button-group>
              <el-button text @click="onTypeButtonClick('question')" :type="searchType==='question'?'primary':''">
                问题
              </el-button>
              <el-button text @click="onTypeButtonClick('article')" :type="searchType==='article'?'primary':''">
                文章
              </el-button>
              <el-button text @click="onTypeButtonClick('user')" :type="searchType==='user'?'primary':''">
                用户
              </el-button>
            </el-button-group>
          </div>
          ...
          <div v-if="searchType==='question'">
            <!--    问题列表-->
            <div class="list" v-for="item in searchResultData" :key="item.id">
            ...
            <!--加载更多按钮-->
            <div style="display: flex;justify-content: center;margin-top: 16px" v-if="searchCurrentPage!==-1">
          ...
          <div v-if="searchType==='user'">
            <div class="list">
              <el-row>
                <el-col :span="12" v-for="item in searchResultData" :key="item.id" class="preview-card">
                  <el-row>
                    <el-col :span="4">
      ...
      <!--      如果没有搜索-->
      <el-empty description="未输入搜索内容" v-if="searchText.length<=0"/>
